...
var getChainInfo = async function(peer, username, org) {
	var target = buildTarget(peer, org);
	var channel = helper.getChannelForOrg(org);
...
var getChainInfo = async function(peer, username, org) {
	...
	return helper.getRegisteredUsers(username, org).then((member) => {
		return await channel.queryInfo(target);
	}
};
*/
...
var getBlockTimestamp = async function(peer, blockNumber, username, orgname) {
	var blockTimestamp = null;

	return getBlockByNumber(peer, blockNumber, username, orgname).then(function(block) {
		if (block && block.data && block.data.data[0]) {
			let first_tx = block.data.data[0];
			let header = first_tx.payload.header;
			blockTimeStamp = Date.parse(header.channel_header.timestamp);
		} 
...
var getBlockNumByTime = async function (peer, startDate, endDate, username, orgname) {
	try {
		var res = await getChainInfo(peer, username, orgname)
		var maxBlockNum = res["height"]["low"]
	} catch (err) {
		return 'Failed to send query due to error: ' + err.stack ? err.stack : err;
	};
	logger.debug('maxBlockNum: ' + maxBlockNum)


	var START = 0;
	var END = maxBlockNum - 1;
	var blockEndNum = END;
	var blockStartNum = 0;
	
	var firstBlockTime = await getBlockTimestamp(peer, 0, username, orgname)
	logger.debug('first: ' + firstBlockTime)
	logger.debug('startDate: ' + startDate)
	logger.debug('endDate: ' + endDate)


	if (endDate < firstBlockTime) {
		return 0
	}
	while (END >= START) {
		let currentBlockTime = await getBlockTimestamp(peer, END, username, orgname);	
		logger.debug('currentBlockTime: ' + currentBlockTime)
	
		if (endDate >= currentBlockTime) {
			blockEndNum = END;
			break;
		}
		END--;
	}
	if (startDate <= firstBlockTime)
		return blockEndNum + 1

	while (END >= START) {
		let currentBlockTime = await getBlockTimestamp(peer, END, username, orgname);	
		if (startDate > currentBlockTime) {
			blockStartNum = END;
			break;
		}
		END--;
	}
	logger.debug('blockEndNum: ' + blockEndNum)
	logger.debug('blockStartNum: ' + blockStartNum)

	return blockEndNum - blockStartNum
};
...
exports.getBlockNumByTime = getBlockNumByTime;